...
namespace Mudbase.SDK.Test.Api
{
    ...
    /// </summary>
    public sealed class MCPApiTests : ApiTestsBase
    {
        private readonly IMCPApi _instance;

        public MCPApiTests(): base(Array.Empty<string>())
        {
            _instance = _host.Services.GetRequiredService<IMCPApi>();
        }
        ...
        public async Task McpConfigGetAsyncTest()
        {
            var response = await _instance.McpConfigGetAsync();
            var model = response.Ok();
            Assert.IsType<McpConfigGet200Response>(model);
        }
    }
}
