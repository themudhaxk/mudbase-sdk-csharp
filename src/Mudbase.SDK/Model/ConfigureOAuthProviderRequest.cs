/*
 * MUDBASESDK
 *
 * MUDBASE is a scalable, real-time, and secure Backend-as-a-Service (BaaS) platform  designed for modern applications. Built with custom logic, it offers fine-grained  control, extensibility, and enterprise-grade security.  ## Features - 🔐 Multi-provider authentication (30+ OAuth providers) - 📊 Real-time database with collections - 📁 File storage and management - 🔑 API key management with permissions - 🔗 Webhook system with retry logic - ⚡ Serverless functions - 💬 Multi-channel messaging (Push, Email, SMS) - 📈 Usage analytics and monitoring - 🌐 Real-time WebSocket events - 🔍 Full-text search capabilities - 💳 Billing: fiat only for project subscriptions and org BaaS checkout (platform fee split). On-chain billing is not exposed on these APIs (optional `crypto-payment-module/` in repo, not mounted by default). - 🏢 Enterprise / Phase 4: custom domains on Growth, Scale, and Enterprise (TXT DNS at `_mudbase-verify.<hostname>`); `settings.customDomainAddon` is optional (billing/legacy); dedicated DB migration script, periodic DNS recheck job, optional `infrastructureEnvironments[]` and edge/metering fields on `dedicated`. 
 *
 * The version of the OpenAPI document: 1.3.16
 * Contact: support@mudbase.dev
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using FileParameter = Mudbase.SDK.Client.FileParameter;
using OpenAPIDateConverter = Mudbase.SDK.Client.OpenAPIDateConverter;

namespace Mudbase.SDK.Model
{
    /// <summary>
    /// ConfigureOAuthProviderRequest
    /// </summary>
    [DataContract(Name = "configureOAuthProvider_request")]
    public partial class ConfigureOAuthProviderRequest : IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="ConfigureOAuthProviderRequest" /> class.
        /// </summary>
        [JsonConstructorAttribute]
        protected ConfigureOAuthProviderRequest() { }
        /// <summary>
        /// Initializes a new instance of the <see cref="ConfigureOAuthProviderRequest" /> class.
        /// </summary>
        /// <param name="enabled">Whether the OAuth provider is enabled (required).</param>
        /// <param name="clientId">OAuth client ID from the provider (required).</param>
        /// <param name="clientSecret">OAuth client secret from the provider (required).</param>
        /// <param name="scope">OAuth scopes to request.</param>
        /// <param name="displayName">Custom display name for the provider.</param>
        public ConfigureOAuthProviderRequest(bool enabled = default, string clientId = default, string clientSecret = default, List<string> scope = default, string displayName = default)
        {
            this.Enabled = enabled;
            // to ensure "clientId" is required (not null)
            if (clientId == null)
            {
                throw new ArgumentNullException("clientId is a required property for ConfigureOAuthProviderRequest and cannot be null");
            }
            this.ClientId = clientId;
            // to ensure "clientSecret" is required (not null)
            if (clientSecret == null)
            {
                throw new ArgumentNullException("clientSecret is a required property for ConfigureOAuthProviderRequest and cannot be null");
            }
            this.ClientSecret = clientSecret;
            this.Scope = scope;
            this.DisplayName = displayName;
        }

        /// <summary>
        /// Whether the OAuth provider is enabled
        /// </summary>
        /// <value>Whether the OAuth provider is enabled</value>
        /*
        <example>true</example>
        */
        [DataMember(Name = "enabled", IsRequired = true, EmitDefaultValue = true)]
        public bool Enabled { get; set; }

        /// <summary>
        /// OAuth client ID from the provider
        /// </summary>
        /// <value>OAuth client ID from the provider</value>
        /*
        <example>123456789-abcdefghijklmnop.apps.googleusercontent.com</example>
        */
        [DataMember(Name = "clientId", IsRequired = true, EmitDefaultValue = true)]
        public string ClientId { get; set; }

        /// <summary>
        /// OAuth client secret from the provider
        /// </summary>
        /// <value>OAuth client secret from the provider</value>
        /*
        <example>GOCSPX-abcdefghijklmnopqrstuvwxyz</example>
        */
        [DataMember(Name = "clientSecret", IsRequired = true, EmitDefaultValue = true)]
        public string ClientSecret { get; set; }

        /// <summary>
        /// OAuth scopes to request
        /// </summary>
        /// <value>OAuth scopes to request</value>
        /*
        <example>[&quot;profile&quot;,&quot;email&quot;]</example>
        */
        [DataMember(Name = "scope", EmitDefaultValue = false)]
        public List<string> Scope { get; set; }

        /// <summary>
        /// Custom display name for the provider
        /// </summary>
        /// <value>Custom display name for the provider</value>
        /*
        <example>Sign in with Google</example>
        */
        [DataMember(Name = "displayName", EmitDefaultValue = false)]
        public string DisplayName { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class ConfigureOAuthProviderRequest {\n");
            sb.Append("  Enabled: ").Append(Enabled).Append("\n");
            sb.Append("  ClientId: ").Append(ClientId).Append("\n");
            sb.Append("  ClientSecret: ").Append(ClientSecret).Append("\n");
            sb.Append("  Scope: ").Append(Scope).Append("\n");
            sb.Append("  DisplayName: ").Append(DisplayName).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
