/*
 * MUDBASESDK
 *
 * MUDBASE is a scalable, real-time, and secure Backend-as-a-Service (BaaS) platform  designed for modern applications. Built with custom logic, it offers fine-grained  control, extensibility, and enterprise-grade security.  ## Features - 🔐 Multi-provider authentication (30+ OAuth providers) - 📊 Real-time database with collections - 📁 File storage and management - 🔑 API key management with permissions - 🔗 Webhook system with retry logic - ⚡ Serverless functions - 💬 Multi-channel messaging (Push, Email, SMS) - 📈 Usage analytics and monitoring - 🌐 Real-time WebSocket events - 🔍 Full-text search capabilities - 💳 Billing: fiat only for project subscriptions and org BaaS checkout (platform fee split). On-chain billing is not exposed on these APIs (optional `crypto-payment-module/` in repo, not mounted by default). - 🏢 Enterprise / Phase 4: custom domains on Growth, Scale, and Enterprise (TXT DNS at `_mudbase-verify.<hostname>`); `settings.customDomainAddon` is optional (billing/legacy); dedicated DB migration script, periodic DNS recheck job, optional `infrastructureEnvironments[]` and edge/metering fields on `dedicated`. 
 *
 * The version of the OpenAPI document: 1.3.16
 * Contact: support@mudbase.dev
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using FileParameter = Mudbase.SDK.Client.FileParameter;
using OpenAPIDateConverter = Mudbase.SDK.Client.OpenAPIDateConverter;

namespace Mudbase.SDK.Model
{
    /// <summary>
    /// WebPushSubscriptionKeys
    /// </summary>
    [DataContract(Name = "WebPushSubscription_keys")]
    public partial class WebPushSubscriptionKeys : IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="WebPushSubscriptionKeys" /> class.
        /// </summary>
        [JsonConstructorAttribute]
        protected WebPushSubscriptionKeys() { }
        /// <summary>
        /// Initializes a new instance of the <see cref="WebPushSubscriptionKeys" /> class.
        /// </summary>
        /// <param name="p256dh">The subscription&#39;s P-256 ECDH public key (base64url). (required).</param>
        /// <param name="auth">The subscription&#39;s auth secret (base64url). (required).</param>
        public WebPushSubscriptionKeys(string p256dh = default, string auth = default)
        {
            // to ensure "p256dh" is required (not null)
            if (p256dh == null)
            {
                throw new ArgumentNullException("p256dh is a required property for WebPushSubscriptionKeys and cannot be null");
            }
            this.P256dh = p256dh;
            // to ensure "auth" is required (not null)
            if (auth == null)
            {
                throw new ArgumentNullException("auth is a required property for WebPushSubscriptionKeys and cannot be null");
            }
            this.Auth = auth;
        }

        /// <summary>
        /// The subscription&#39;s P-256 ECDH public key (base64url).
        /// </summary>
        /// <value>The subscription&#39;s P-256 ECDH public key (base64url).</value>
        [DataMember(Name = "p256dh", IsRequired = true, EmitDefaultValue = true)]
        public string P256dh { get; set; }

        /// <summary>
        /// The subscription&#39;s auth secret (base64url).
        /// </summary>
        /// <value>The subscription&#39;s auth secret (base64url).</value>
        [DataMember(Name = "auth", IsRequired = true, EmitDefaultValue = true)]
        public string Auth { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class WebPushSubscriptionKeys {\n");
            sb.Append("  P256dh: ").Append(P256dh).Append("\n");
            sb.Append("  Auth: ").Append(Auth).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
