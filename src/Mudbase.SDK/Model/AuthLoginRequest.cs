...
namespace Mudbase.SDK.Model
{
    ...
    public partial class AuthLoginRequest : IValidatableObject
    {
        ...
        [JsonConstructor]
        public AuthLoginRequest(string email, string password, Option<string?> projectId = default)
        {
            Email = email;
            Password = password;
    ...
    public class AuthLoginRequestJsonConverter : JsonConverter<AuthLoginRequest>
    {
        ...
        public override AuthLoginRequest Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (!email.IsSet)
                throw new ArgumentException("Property is required for class AuthLoginRequest.", nameof(email));

            if (!password.IsSet)
                throw new ArgumentException("Property is required for class AuthLoginRequest.", nameof(password));

            if (email.IsSet && email.Value == null)
                throw new ArgumentNullException(nameof(email), "Property is not nullable for class AuthLoginRequest.");
            ...
            if (projectId.IsSet && projectId.Value == null)
                throw new ArgumentNullException(nameof(projectId), "Property is not nullable for class AuthLoginRequest.");

            return new AuthLoginRequest(email.Value!, password.Value!, projectId);
        }
        ...
        {
            if (authLoginRequest.Email == null)
                throw new ArgumentNullException(nameof(authLoginRequest.Email), "Property is required for class AuthLoginRequest.");
            ...
            if (authLoginRequest.ProjectIdOption.IsSet && authLoginRequest.ProjectId == null)
                throw new ArgumentNullException(nameof(authLoginRequest.ProjectId), "Property is required for class AuthLoginRequest.");

            writer.WriteString("email", authLoginRequest.Email);

            writer.WriteString("password", authLoginRequest.Password);

            if (authLoginRequest.ProjectIdOption.IsSet)
                writer.WriteString("projectId", authLoginRequest.ProjectId);
        }
    }
}
